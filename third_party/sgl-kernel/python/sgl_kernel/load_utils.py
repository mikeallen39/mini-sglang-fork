...
def _load_architecture_specific_ops():
    ...
    attempted_found_files: List[List[str]] = []

    # Both sm90 and sm100 builds are fatbins that may contain kernels for
    # multiple architectures. On older GPUs such as sm80, preferring the sm90
    # variant is more conservative than incorrectly forcing the sm100 path.
    if compute_capability is None:
        candidate_variants = [
            ("sm90", "No GPU detected; trying SM90 common_ops first"),
            ("sm100", "No GPU detected; trying SM100 common_ops fallback"),
        ]
    elif compute_capability >= 100:
        candidate_variants = [
            ("sm100", f"SM{compute_capability} precise-math common_ops"),
            ("sm90", f"SM{compute_capability} fallback to SM90 common_ops"),
        ]
    else:
        candidate_variants = [
            ("sm90", f"SM{compute_capability} compatibility path via SM90 common_ops"),
            ("sm100", f"SM{compute_capability} fallback to SM100 common_ops"),
        ]

    for ops_subdir, variant_name in candidate_variants:
        ...
        attempted_patterns.append(ops_pattern)
        attempted_found_files.append(matching_files)
...
{chr(10).join(f"   - {pattern} -> {files}" for pattern, files in zip(attempted_patterns, attempted_found_files))}
